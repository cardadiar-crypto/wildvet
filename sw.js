...
self.addEventListener('fetch', e => {
  ...
  // No interceptar Firebase, APIs ni OAuth
  ...
  // CDN — Cache First
  const isCDN = CDN_ASSETS.some(a => url.split('?')[0].startsWith(a.split('?')[0]));
  if (isCDN) {
    e.respondWith(
  ...
  // App Shell — /wildvet y /wildvet/* — Network First con fallback
